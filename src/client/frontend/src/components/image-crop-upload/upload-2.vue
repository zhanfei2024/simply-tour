  ...
  export default {
    ...
    methods: {
      ...
      createImg(e) {
        ...
        store.commit('setFilesToUpload', {field: field, createImgUrl: that.createImgUrl});
      },
      prepareUpload() {
        let {
          url,
        ...
        if (typeof url == 'string' && url || typeof this.myUrl == 'string' && this.myUrl) {
          this.upload();
        } else {
          this.off();
        }
      ...
      upload() {
        ...
        store.state.filesToUpload.forEach((currentValue) => {
          fmData.append(currentValue.field, data2blob(currentValue.createImgUrl, mime), currentValue.field + '.' + imgFormat);
        });
